 ...
 * 社区评比列表界面
 */
public class CommunityRatingActivity extends BannerBaseActivity {
	...
	private void initData() {
		...
		listView.setGetDataCallBack(new HttpBaseCallBack<JSONObject>(getBaseContext(), showList, adapter, listView, loadingLayout,
				JSONObject.class, GlobalUrl.GET_COMMUNITY_LIST_DATA, "communityType", "4"));
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
				...
				if (position >= 0) {
					...
					IntentUtils.goTo(getActivity(), CommunityRatingContentActivity.class, extras);
				}
			}
		});
	}
